  ...
  async function confirmPasswordReset() {
    ...
    try {
      await api("/public/raffles/auth/password/reset", {
        method: "POST",
        body: JSON.stringify({ login, code: resetCode, newPassword })
      });
              ...
              <form onSubmit={submitLogin} className="mt-5 grid gap-3">
                ...
                {!recoveringPassword ? (
                  ...
                  <div className="grid gap-3 rounded-2xl border border-orange-200 bg-orange-50 p-4">
                    <p className="text-sm text-orange-950">Digite o codigo de 6 digitos enviado ao seu WhatsApp. Ele vale por 15 minutos.</p>
                    <input
                      value={resetCode}
                    ...
                    />
                    <button type="button" disabled={resetLoading} onClick={() => void confirmPasswordReset()} className="rounded-xl bg-ember px-5 py-3 font-bold text-white disabled:opacity-60">
