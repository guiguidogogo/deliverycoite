...
export function IptvAdminPanel() {
  ...
  const [subscriberCredentials, setSubscriberCredentials] = useState(emptyCredentials);
  const [credentialsSubscriberId, setCredentialsSubscriberId] = useState<string | null>(null);
  ...
  function openSubscriberCredentials(item: Subscriber) {
    setCredentialsSubscriberId((current) => current === item.id ? null : item.id);
    setSubscriberCredentials(emptyCredentials);
  }

  async function saveSubscriberCredentials(item: Subscriber) {
    if (!subscriberCredentials.server.trim() || !subscriberCredentials.username.trim() || !subscriberCredentials.password) {
      return toast.error("Informe o host, o usuario e a senha do IPTV");
    }
    setSavingCredentials(true);
    try {
      await adminApi<Subscriber>(`/admin/my-app/subscribers/${item.id}/credentials`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(subscriberCredentials)
      });
      setSubscriberCredentials(emptyCredentials);
      setCredentialsSubscriberId(null);
      toast.success(`Dados IPTV de ${item.name} atualizados`);
      await load();
    } catch (error) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {loading ? <div className="mt-8 rounded-3xl bg-white/10 p-8">Carregando licenças...</div> : !subscription ? (
          ...
          <section className="mt-8 grid gap-4 md:grid-cols-4">
            ...
            <InfoCard label="Listas configuradas" value={`${subscribers.filter((item) => item.configured).length}/${subscribers.length}`} tone={subscribers.length > 0 && subscribers.every((item) => item.configured) ? "green" : "amber"} />
          ...
          <section className="mt-6 rounded-3xl border border-white/10 bg-white/10 p-6 backdrop-blur">
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-2">
              ...
              {subscribers.map((item) => <article key={item.id} className="rounded-2xl border border-white/10 bg-black/20 p-5">
                ...
                <div className="mt-4 grid grid-cols-3 gap-3 text-sm"><div><p className="text-white/50">Plano</p><p className="font-semibold">{planText[item.plan]}</p></div><div><p className="text-white/50">Validade</p><p className="font-semibold">{formatDate(item.expiresAt)}</p></div><div><p className="text-white/50">Lista IPTV</p><p className={`font-semibold ${item.configured ? "text-emerald-300" : "text-amber-300"}`}>{item.configured ? "Configurada" : "Pendente"}</p></div></div>
                ...
                <div className="mt-5 flex flex-wrap gap-2"><button className="rounded-lg bg-emerald-600 px-3 py-2 text-sm font-semibold" onClick={() => openSubscriberCredentials(item)}>{item.configured ? "Alterar IPTV" : "Configurar IPTV"}</button><button className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold" onClick={() => editSubscriber(item)}>Editar / renovar</button><button className={`rounded-lg px-3 py-2 text-sm font-semibold ${item.active ? "bg-amber-600" : "bg-emerald-600"}`} onClick={() => void toggleSubscriber(item)}>{item.active ? "Bloquear" : "Liberar"}</button><button className="rounded-lg bg-violet-600 px-3 py-2 text-sm font-semibold" onClick={() => void regenerateCode(item)}>Novo código</button><button className="rounded-lg bg-fuchsia-600 px-3 py-2 text-sm font-semibold" onClick={() => { setPairingSubscriberId(item.id); setTvCode(""); }}>Vincular código da TV</button><button className="rounded-lg bg-red-700 px-3 py-2 text-sm font-semibold" onClick={() => void removeSubscriber(item)}>Excluir</button></div>
                {credentialsSubscriberId === item.id && <div className="mt-4 rounded-xl border border-emerald-300/30 bg-emerald-500/10 p-4">
                  <p className="font-semibold">Lista IPTV exclusiva de {item.name}</p>
                  <p className="mt-1 text-sm text-white/60">O aplicativo receberá estes dados quando este código de acesso for ativado.</p>
                  <div className="mt-3 grid gap-3 md:grid-cols-3">
                    <Field label="Host / servidor"><input type="url" placeholder="http://servidor.com:8080" value={subscriberCredentials.server} onChange={(event) => setSubscriberCredentials({ ...subscriberCredentials, server: event.target.value })} autoComplete="off" /></Field>
                    <Field label="Login / usuário"><input placeholder="Usuário IPTV" value={subscriberCredentials.username} onChange={(event) => setSubscriberCredentials({ ...subscriberCredentials, username: event.target.value })} autoComplete="off" /></Field>
                    <Field label="Senha"><input type="password" placeholder="Senha IPTV" value={subscriberCredentials.password} onChange={(event) => setSubscriberCredentials({ ...subscriberCredentials, password: event.target.value })} autoComplete="new-password" /></Field>
                  </div>
                  <p className="mt-2 text-xs text-white/50">Por segurança, os dados atuais não são exibidos. Salvar substitui os três dados deste assinante.</p>
                  <div className="mt-3 flex gap-2"><button disabled={savingCredentials} className="rounded-xl bg-emerald-600 px-5 py-3 font-bold disabled:opacity-50" onClick={() => void saveSubscriberCredentials(item)}>{savingCredentials ? "Salvando..." : "Salvar lista deste assinante"}</button><button className="rounded-xl bg-white/10 px-4 py-3" onClick={() => setCredentialsSubscriberId(null)}>Cancelar</button></div>
                </div>}
