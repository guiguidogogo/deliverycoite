...
type ExistingOrderItem = {
  productId: string;
  quantity: number;
  complements?: Array<{ complementId: string | null; quantity: number }>;
};
...
export async function restoreStockFromOrderItems(tx: Tx, companyId: string, items: ExistingOrderItem[]) {
  const productQuantities = new Map<string, number>();
  const complementQuantities = new Map<string, number>();
  ...
  await Promise.all([
    ...products.map((product) => tx.product.update({
      where: { id: product.id },
      data: { stockQuantity: { increment: new Prisma.Decimal(productQuantities.get(product.id) ?? 0) } }
    })),
    ...complements.map((complement) => tx.complement.update({
      where: { id: complement.id },
      data: { stockQuantity: { increment: new Prisma.Decimal(complementQuantities.get(complement.id) ?? 0) } }
    }))
  ]);
}
